...
function phanTich_URL_chiTietSanPham() {
    ...
    // tách theo dấu '-' vào gắn lại bằng dấu ' ', giúp bỏ hết dấu '-' thay vào bằng khoảng trắng.
    // chỗ code này làm ngược lại so với lúc tạo href cho sản phẩm trong file classes.js
    ...
    // Đổi title
    document.title = nameProduct + ' - SMARTPHONE VERSE';
    ...
    if (sanPham.promo.name != 'giareonline') {
        ...
        document.getElementsByClassName('ship')[0].style.display = ''; // hiển thị 'giao hàng trong n giờ'
...
function getDetailPromo(sp) {
    switch (sp.promo.name) {
        ...
        case 'moiramat':
            return `Khách hàng sẽ được thử máy miễn phí tại cửa hàng. Có thể đổi trả lỗi trong vòng 6 tháng.`;

        case 'giareonline':
            var del = stringToNum(sp.price) - stringToNum(sp.promo.value);
            var span = `<span style="font-weight: bold">` + numToString(del) + `</span>`;
            return `Sản phẩm sẽ được giảm ` + span + `₫ khi mua hàng bằng thẻ Visa/Master Card`;
